@props(['formAttr' => []])
<form {{ $attributes }} class="main-content w-full px-[var(--margin-x)] pb-8">
    <div class="flex items-center space-x-4 py-5 lg:py-6">
        ...
        <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
            <li class="flex items-center space-x-2">
                @if (\Route::has(data_get($formAttr, 'routeEdit')))
                    <a class="text-primary transition-colors hover:text-primary-focus dark:text-accent-light dark:hover:text-accent"
                        href="{{ route(data_get($formAttr, 'routeEdit', 'admin')) }}">
                        {{ data_get($formAttr, 'description', 'Current') }}
                    </a>
                @elseif(\Route::has(data_get($formAttr, 'routeList')))
                    <a class="text-primary transition-colors hover:text-primary-focus dark:text-accent-light dark:hover:text-accent"
                        href="{{ route(data_get($formAttr, 'routeList', 'admin')) }}">
                        {{ data_get($formAttr, 'description', 'Current') }}
                    </a>
                @else
                    <a class="text-primary transition-colors hover:text-primary-focus dark:text-accent-light dark:hover:text-accent"
    ...
    <div class="grid grid-cols-12 gap-4 sm:gap-5 lg:gap-6">
        ...
        @endisset
        <div class="col-span-12 lg:col-span-{{ data_get($formAttr, 'span', '12') }} ">
            <div class="card">
                <div
                    ...
                    <div class="flex justify-center space-x-2">
                        @if (\Route::has(data_get($formAttr, 'routeList')))
                            <a href="{{ route(data_get($formAttr, 'routeList')) }}"
